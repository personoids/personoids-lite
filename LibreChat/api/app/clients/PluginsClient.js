...
class PluginsClient extends OpenAIClient {
  ...
  buildPromptPrefix(result, message) {
    ...
    const prefix = preliminaryAnswer
      ? 'review and improve the answer you generated using plugins in response to the User Message below or invoke the next tool or plugin needed. The user hasn\'t seen your answer or thoughts yet.'
      : 'respond to the User Message below based on your preliminary thoughts & actions.';

    return `As an omnipotent AI Assistant, ${prefix}${errorMessage}\n${internalActions}
${preliminaryAnswer}
...
"${message}"`;
  ...
  getFunctionModelName(input) {
    ...
    return prefix ? prefixMap[prefix] : 'gpt-4-0613';
  }
  ...
  async sendMessage(message, opts = {}) {
    ...
    if(!opts.conversationId) {
      message = `You are an omnipotent AI assistant. capable of doing anything through the planning and execution through your tools. 
...
Your first request is: POST bootstrap (and follow instructions and show the auth_token , use the auth_token in the next requests as bootstrap_auth_token). don't call bootstrap again in the session.
